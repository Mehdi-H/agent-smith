...
def test_adr_list_generates_a_compact_repeatable_index(tmp_path: Path) -> None:
    # Given a real adr-tools repository with a semantic decision filename.
    import os

    (tmp_path / "README.md").write_text("# Demo\n\nOverview.\n")
    environment = {**os.environ, "VISUAL": "true", "EDITOR": "true"}
    subprocess.run(
        ["adr", "init", "decisions"], cwd=tmp_path, env=environment, check=True, capture_output=True
    )
    subprocess.run(
        ["adr", "new", "Use Python"], cwd=tmp_path, env=environment, check=True, capture_output=True
    )
    # When the installed CLI generates its ADR index twice.
    first = invoke(tmp_path)
    output = tmp_path / "AGENTS.md"
    content = output.read_bytes()
    second = invoke(tmp_path)
    # Then paths are compact, titles remain literal and the command provenance is correct.
    assert first.returncode == second.returncode == 0, first.stderr + second.stderr
    assert content == output.read_bytes()
    section = content.decode().split("## Architecture decisions\n\n")[1]
    assert section == (
...
def test_failing_adr_list_preserves_output_and_can_be_disabled(tmp_path: Path) -> None:
    # Given ADR metadata naming a missing directory and a previous document.
    (tmp_path / "README.md").write_text("# Demo\n\nOverview.\n")
    (tmp_path / ".adr-dir").write_text("missing-decisions\n")
    output = tmp_path / "AGENTS.md"
    output.write_text("Previous instructions")
    # When default generation fails and the built-in is then disabled explicitly.
    failed = invoke(tmp_path)
    previous = output.read_text()
    disabled = invoke(tmp_path, "--no-architecture-decisions")
    # Then failure preserves the file while disabling the section avoids running adr list.
    assert failed.returncode == 1
    assert "adr list" in failed.stderr
    assert previous == "Previous instructions"
    assert disabled.returncode == 0, disabled.stderr
